...
class DatabaseManager:
    ...
    def get_winners_count(self, prize_id):
        ...
        with conn:
            ...
            ''', (prize_id,))
            return cur.fetchone()[0]  # fetchall yerine fetchone kullanarak doğrudan değeri alıyoruz.
    ...
    def get_rating(self):
        ...
        with conn:
            cur = conn.cursor()
            cur.execute('''
                SELECT users.name, COUNT(winners.user_id) AS total_wins
                FROM users
                LEFT JOIN winners ON users.id = winners.user_id
                GROUP BY users.id
